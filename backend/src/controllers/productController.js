...
export const addProduct = async (req, res) => {
  const { name, description, price, category, glutenFree, vegan, seasonal } = req.body;
  const images = req.files.map((file) => file.path);

  try {
    const product = await Product.create({ name, description, price, images, category, glutenFree, vegan, seasonal });
